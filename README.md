...
面向普通读者的开源 Windows AI 新闻流。首页每批固定显示 10 条，可点击“换一批”，并优先保证大模型、Agent、重要研究、开源项目和产业动态都有覆盖。卡片只显示「标题 + 摘要」；点开后先读核心结论、领域定位和 2–4 个上下文术语，再按资讯类型阅读带小标题的完整报道和原始来源。
...
- 公共内容池目标为 20–40 条，首页每批固定 10 条；每一批都必须包含大模型、Agent、论文和开源项目。
...
- 每条详情在正文前必须用一句话说明“这是哪个领域、正在解决什么问题”，并解释 2–4 个真正影响理解的术语；术语解释必须结合当前事件，不能复制百科定义。
...
资讯详情页提供“使用 Codex 深度分析”和“打开原始网站”两个入口。深度分析会在应用内打开独立的 Codex 阅读工作区，不再弹出命令行窗口；回答会流式显示，用户可以继续追问、停止生成或开始新对话。切换到另一条资讯后再次分析仍沿用同一会话，便于比较不同事件。应用会从本机 Codex 实际提供的模型中自动选择兼容版本，避免旧 CLI 因账户默认模型过新而无法回答。
