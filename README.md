...
资讯详情页提供“使用 Codex 深度分析”和“打开原始网站”两个入口。深度分析会打开完全不透明的原生 Codex 阅读窗口，可自由移动、拖边缩放、最小化或最大化，不再弹出命令行窗口；回答会流式显示，用户可以继续追问、停止生成或开始新对话。关闭阅读窗口只会暂时隐藏它，切换到另一条资讯后再次分析仍沿用同一会话，便于比较不同事件。
